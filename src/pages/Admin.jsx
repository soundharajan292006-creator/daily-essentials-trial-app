...
const StatCard = ({ title, value, icon: Icon, color }) => (
  <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center">
    <div className={`w-14 h-14 rounded-xl flex items-center justify-center mr-5 ${color}`}>
...
const navItems = [
  ...
  { id: 'trials', label: 'Trial Requests', icon: ShoppingCart },
...
const Admin = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      if (activeTab === 'dashboard') {
        ...
        setStats({
          totalProducts: Number(data.total_products ?? data.totalProducts ?? 0),
          ...
          // Backend currently returns a mock trial count.
          // Do not display it as a verified database count.
          totalTrials: null,

          // Show reviews count only when the backend provides it.
          totalReviews:
        ...
        setProducts(getArray(res, 'products'));
      } else if (activeTab === 'categories') {
        const res = await categoryService.getCategories();
        setCategories(getArray(res, 'categories'));
      } else if (activeTab === 'trials') {
        const res = await adminService.getTrials();
        setTrials(getArray(res, 'trials'));
      } else if (activeTab === 'orders') {
        const res = await adminService.getOrders();
        setOrders(getArray(res, 'orders'));
      } else if (activeTab === 'users') {
        const res = await adminService.getUsers();
        setUsers(getArray(res, 'users'));
      } else if (activeTab === 'reviews') {
        const res = await adminService.getReviews();
        setReviews(getArray(res, 'reviews'));
      }
    } catch (err) {
  ...
  const handleTrialStatus = async (id, status) => {
    try {
      await adminService.updateTrialStatus(id, status);
      await fetchData();
    } catch (err) {
      setError(err?.response?.data?.message || 'Failed to update trial.');
    }
  };
      ...
      <div className="flex-1 p-4 md:p-8 overflow-y-auto min-w-0">
        ...
        {!loading && activeTab === 'dashboard' && (
          <div className="space-y-8">
            ...
            <p className="text-xs text-gray-400">
              Revenue includes paid orders only. A dash means the backend
              does not yet provide a verified count.
            </p>
          </div>
        )}
        ...
        {!loading && activeTab === 'products' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <button
                disabled
                title="Product creation will be connected after backend verification"
            ...
            <TableWrapper>
              <TableHead columns={[
                'Product', 'Category', 'Price (Trial/Full)', 'Stock', 'Actions'
              ]} />

              <tbody className="bg-white divide-y divide-gray-200">
                {products.map((product) => {
                  const id = getId(product, 'product_id');
                  ...
                  return (
                    <tr key={id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600 whitespace-nowrap">
                        ...
                        {formatMoney(product.price ?? product.full_price)}
                      </td>

                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 text-xs font-semibold rounded-full ${stock > 0
                      ...
                      <td className="px-6 py-4 text-sm whitespace-nowrap">
                        ...
                        <button
                          disabled
                          title="Edit API not connected yet"
                        ...
                        <button
                          disabled
                          title="Delete API not connected yet"
                ...
                {products.length === 0 && (
                  <EmptyRow colSpan={5} message="No products found." />
                )}
              </tbody>
            </TableWrapper>
          </div>
        ...
        {!loading && activeTab === 'categories' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <button
                disabled
                title="Category creation not connected yet"
                className="bg-gray-300 text-white px-4 py-2 rounded-lg font-medium flex items-center cursor-not-allowed"
              >
                <Plus className="h-5 w-5 mr-1" />
                Add Category
            ...
            <TableWrapper>
              <TableHead columns={['ID', 'Name', 'Slug', 'Actions']} />
              <tbody className="bg-white divide-y divide-gray-200">
                {categories.map((cat) => {
                  const id = getId(cat, 'category_id');
                  ...
                  return (
                    <tr key={id}>
                      ...
                      <td className="px-6 py-4">
                        <button
                          disabled
                          title="Category editing not connected yet"
                ...
                {categories.length === 0 && <EmptyRow colSpan={4} />}
              </tbody>
            </TableWrapper>
          </div>
        )}
        ...
        {!loading && activeTab === 'users' && (
          <div className="space-y-6">
            ...
            <TableWrapper>
              <TableHead columns={['Name', 'Email', 'Phone', 'Role', 'Joined']} />
              <tbody className="bg-white divide-y divide-gray-200">
                {users.map((user) => (
                  <tr key={user.id}>
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">
        ...
        {!loading && activeTab === 'orders' && (
          <div className="space-y-6">
            ...
            <TableWrapper>
              <TableHead columns={[
                'Order ID', 'User', 'Total', 'Status', 'Date', 'Action'
              ]} />

              <tbody className="bg-white divide-y divide-gray-200">
                {orders.map((order) => {
                  const id = getId(order, 'order_id');
                    ...
                    <tr key={id}>
                      ...
                      <td className="px-6 py-4">
                        <select
                          ...
                          value={status}
                          onChange={(e) => handleOrderStatus(id, e.target.value)}
                        >
        ...
        {!loading && activeTab === 'trials' && (
          <div className="space-y-6">
            ...
            <TableWrapper>
              <TableHead columns={[
                'Request ID', 'Product', 'User', 'Status', 'Date', 'Action'
              ]} />

              <tbody className="bg-white divide-y divide-gray-200">
                {trials.map((trial) => {
                  const id = getId(trial, 'trial_id') ?? trial.request_id;
                  const status = trial.request_status || trial.status || 'pending';
                    ...
                    <tr key={id}>
                      ...
                      <td className="px-6 py-4">
                        <select
                          className="border border-gray-300 rounded text-sm px-2 py-1"
                          value={status}
                          onChange={(e) => handleTrialStatus(id, e.target.value)}
                        >
                ...
                {trials.length === 0 && <EmptyRow colSpan={6} />}
              </tbody>
            </TableWrapper>
          </div>
        )}
        ...
        {!loading && activeTab === 'reviews' && (
          <div className="space-y-6">
            ...
            <TableWrapper>
              <TableHead columns={[
                'Product', 'User', 'Rating', 'Comment', 'Action'
              ]} />

              <tbody className="bg-white divide-y divide-gray-200">
                {reviews.map((review) => {
                  const id = getId(review, 'review_id');
                ...
                {reviews.length === 0 && <EmptyRow colSpan={5} />}
              </tbody>
            </TableWrapper>
          </div>
        )}
